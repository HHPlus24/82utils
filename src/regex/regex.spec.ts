...
describe('regexBuilder', () => {
  ...
  it('should match a valid  Korean pattern' , () => {
    ...
    const test = regexBuilder.add({ type: 'language', count: 3, lang: 'ko' });
  ...
  it ('should match a valid korean and english pattern', () => {
    regexBuilder.add({ type: 'language',count: 3, lang: 'ko' }).add({ type: 'language', count: 2, lang:'en' })
  ...
  it ('should not use an invalid language code', () => {
    regexBuilder.add({ type: 'language', count: 3, lang: 'fr'as any });

    const testString = 'Bonjour';
    const result = regexBuilder.test(testString);
    expect(result).toBeFalsy();
  })
})