  ...
  export function toFormat(date: Date, split: "-" | "/"): string {
    return split == "/" ? `${date.getFullYear()}/${(date.getMonth() + 1).toString().padStart(2, '0')}/${date.getDate().toString().padStart(2, '0')}`:
  ...
  export function add(date: Date, option :{type: "Y" | "M" | "D" | "h" | "m" | "s", inc: number}): Date {
    switch (option.type) {
      case "Y":
  ...
  export function sub(date: Date, option :{type: "Y" | "M" | "D" | "h" | "m" | "s", dec: number}): Date {
    switch (option.type) {
      case "Y":
